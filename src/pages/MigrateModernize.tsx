    ...
    <div className="grid grid-cols-[minmax(0,1fr)_340px] gap-5">
      <div className="space-y-5">
        ...
        <section className="card p-4">
          ...
          <div className="grid grid-cols-5 gap-3">{visible.map((program) => <article key={program.name} className="rounded-xl border border-slate-200 p-4"><h3 className="min-h-10 text-sm font-bold text-slate-950">{program.name}</h3><div className="mt-2 text-xs text-slate-500">{program.source} → {program.target}</div><div className="mt-3 flex items-center justify-between"><span className="text-xs">{program.owner}</span><StatusBadge status={program.status} /></div><div className="mt-4"><ProgressBar value={program.progress} /><div className="mt-1 text-right text-xs font-bold">{program.progress}%</div></div><div className="mt-3 grid grid-cols-3 gap-1 text-center text-xs text-slate-500"><span>Assets<br /><b>{program.assets}</b></span><span>Converted<br /><b>{program.converted}</b></span><span>Score<br /><b>{program.validationScore ?? "-"}</b></span></div><button onClick={() => notify(`${program.name} opened`)} className="btn-secondary mt-4 w-full">{program.status === "Validation" || program.status === "Completed" ? "Review" : "Open Program"}</button></article>)}</div>
        </section>
