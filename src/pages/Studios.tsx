    ...
    <div className="grid grid-cols-[minmax(0,1fr)_340px] gap-5">
      <div className="space-y-5">
            ...
            return (
              <article key={studio.name} className="card p-4">
                <div className="flex items-start gap-4">
                  <div className="rounded-xl bg-orange-50 p-3 text-orange-600"><Icon className="h-8 w-8" /></div>
        ...
        <section className="card p-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            {studioSessions.map((session) => <div key={session.name} className="rounded-xl border border-slate-200 p-4"><h3 className="font-bold text-slate-950">{session.name}</h3><p className="text-sm text-slate-500">{session.opened}</p><div className="mt-4 flex items-center gap-3"><ProgressBar value={session.progress} color="bg-green-500" /><span className="text-sm font-semibold">{session.progress}%</span></div><button className="mt-3 text-sm font-bold text-orange-600">Resume</button></div>)}
          </div>
        </section>
