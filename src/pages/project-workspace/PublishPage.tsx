  ...
  return (
    <div className="workspace-screen">
      ...
      <div className="publish-workspace-grid">
        ...
        <WorkspacePanel title="D  Serving Endpoints"><table className="workspace-table compact-table endpoint-table"><tbody>{endpoints.map((row) => <tr key={row[0]}><td><strong>{row[0]}</strong></td><td>{row[1]}</td><td><Status>Healthy</Status></td><td><button aria-label={`Copy ${row[0]}`} onClick={() => {navigator.clipboard?.writeText(row[1]); setCopied(row[0]);}}><Copy size={13}/></button><button aria-label={`Open ${row[0]}`}><ExternalLink size={13}/></button></td></tr>)}</tbody></table>{copied && <p className="inline-success">{copied} copied</p>}<TextAction>View all endpoints →</TextAction></WorkspacePanel>
